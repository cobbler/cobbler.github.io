...
See {% linkup title:"Relocating Your Installation" extrameta:2.8.0 %} if you have space problems.
...
This is used to serve up certain scripts to anaconda, such as anamon (See {% linkup title:"Anaconda Monitoring" extrameta:2.8.0 %} for more information on anamon).
...
Various user-scripts to extend Cobbler to perform certain actions can be dropped into subdirectories of this directory. See the {% linkup title:"Triggers" extrameta:2.8.0 %} section for more information.
...
* **modules.conf** - auxilliary config file. controls Cobbler security, and what DHCP/DNS engine is attached, see {% linkup title:"Modules" extrameta:2.8.0 %} for developer-level details, and also {% linkup title:"Security Overview" extrameta:2.8.0 %}. This file is in an INI-style format that can be read by the ConfigParser class.
* **users.digest** - if using the digest authorization module this is where your web app username/passwords live. Refer to the {% linkup title:"Cobbler Web User Interface" extrameta:2.8.0 %} section for more info.

#### /etc/cobbler/power

Here we keep the templates for the various power management modules Cobbler supports. Please refer to the {% linkup title:"Power Management" extrameta:2.8.0 %} section for more details on configuring power management features.
...
This is a directory where modules can be dropped to extend Cobbler without modifying the core. See {% linkup title:"Modules" extrameta:2.8.0 %} for more information.
